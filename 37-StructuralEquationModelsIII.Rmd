...
if (!exists("ivm_psem")) stop("ivm_psem not found. Fit it in an earlier chunk.")

# Extract coefficients
ctab <- piecewiseSEM::coefs(ivm_psem)

# ---- CLEAN COLUMN NAMES ----
names(ctab) <- make.names(names(ctab), unique = TRUE)
ctab <- as.data.frame(ctab)

# Remove intercepts early
ctab <- ctab[ctab$Predictor != "(Intercept)", ]

# Clean numeric columns
ctab$Estimate_num <- suppressWarnings(as.numeric(ctab$Estimate))
ctab$P_num        <- suppressWarnings(as.numeric(ctab$P.Value))

ctab_clean <- ctab[!is.na(ctab$Estimate_num) & !is.na(ctab$P_num), ]
...
ctab_clean$label <- sprintf("%.2f%s",
                            ctab_clean$Estimate_num,
                            sigstars(ctab_clean$P_num))
...
dot <- paste0(
  ...
  paste0(
    "  \"", ctab_clean$Predictor,
    "\" -> \"", ctab_clean$Response,
    "\" [label=\"", ctab_clean$label, "\"];\n",
    collapse = ""
  ),
  "}\n"
)
