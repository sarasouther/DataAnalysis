...
* When a species fundamental niche and realized niche are really different due to factors not included in the model, current habitat projects can be inaccurate. This is a common problem when modeling habitat suitability for wildharvested species, since they are underrepresented in suitable habitat, because those habitats are targeted for harvest.
...
Raster layers are spatially mapped grids comprised of hundreds, thousands, or millions of cells or pixels with values related to a variable assigned to each pixel. The smaller the pixel, the higher the resolution, but this greatly affects processing speed and may exceed computer storage.
...
Luckily, we can pull all of this information from a spatial object, use the CRS function and reproject our data so that we are working with all data using the same CRS.
...
bio <- worldclim_global(var = "bio", version="2.1", res=2.5, path='/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/ClassX_SpeciesDistributionModeling')
#or use a temporary directory: bio <- worldclim_global(var = "bio", version="2.1", res=2.5, path=tempdir())
...
#get future data
#Choose a time period: "2021-2040", "2041-2060", or "2061-2080"
#Choose climate model: One of "ACCESS-CM2", "ACCESSESM1-5", "AWI-CM-1-1-MR", "BCC-CSM2-MR", "CanESM5", "CanESM5-CanOE", "CMCC-ESM2", "CNRM-CM6-1", "CNRM-CM6-1-HR", "CNRMESM2-1", "EC-Earth3-Veg", "EC-Earth3-Veg-LR", "FIO-ESM-2-0", "GFDLESM4", "GISS-E2-1-G", "GISS-E2-1-H", "HadGEM3-GC31-LL", "INM-CM4-8", "INM-CM5-0", "IPSL-CM6A-LR", "MIROC-ES2L", "MIROC6", "MPIESM1-2-HR", "MPI-ESM1-2-LR", "MRI-ESM2-0", "UKESM1-0-LL"
#Choose an emissions scenario: A valid Shared Socio-economic Pathway code: "126", "245", "370" or "585".

biof6180  <- geodata::cmip6_world("CNRM-CM6-1", "585", "2061-2080", var="bioc", res=2.5, path='/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/ClassX_SpeciesDistributionModeling')
#biof6180  <- geodata::cmip6_world("CNRM-CM6-1", "585", "2061-2080", var="bioc", res=2.5, path=tempdir())
```

For our data on species occurrence, we will use data from the Global Biodiversity Information Facility (GBIF), a repository for species occurrence data derived from multiple sources, including citizen science, herbarium and museum collections. The GBIF data are biased by observer behavior, since many observations are derived from citizen science projects. Humans tend to collect data from easily accessed areas around roads, popular hiking trails or congregating areas. One way to reduce bias in presenceonly data is to use spatial thinning to reduce weighting observations from heavily trafficked areas more than observations made in other areas. There is no standard thinning distance, but it is typical to require a minimum of 5 km between observations. If you are working at smaller or larger scales, you could reduce or increase this distance! Also, if you are using presenceabsence data or have employed an unbiased sampling strategy to collect data, you can skip this next step. While there are various quality control measures used by GBIF to ensure high data quality, we will run a few other data cleaning codes to remove anomalous points. This step is not necessary if using nonGBIF data! However, when producing your own spatial datasets, some form of data quality control is necessary.

```{r, echo=FALSE, message=FALSE}
opts_chunk$set(comment = NA, echo=FALSE, message = FALSE, warning = FALSE)

#load libraries
library(tidyverse)
library(rgbif)

species_name <- "Pediocactus simpsonii" 
...
dat <- occ_data(taxonKey=key, hasCoordinate = TRUE, continent='north_america', limit=199999, hasGeospatialIssue = FALSE)$data
...
You are familiar with regression based approaches from other statistical analyses! All regression approaches build upon standard regression models (**Fig. 3**), but differ in subtle ways to address common challenges to data modeling  most frequently issues of nonnormality or heterogeneity of variance. 
...
Regressionbased techniques include:
...
In general, it is recommended to avoid having correlated features in your dataset. Indeed, a group of highly correlated features will not bring additional information to our analyses, but will increase the complexity of the algorithm, thus increasing the risk of errors. 
...
# Convert occurdata2 to SpatVector if it's not already
occurdata2_terra <- terra::vect(occurdata2)

# Set the geographic extent using the extent of occurdata2
geographic.extent <- terra::ext(occurdata2_terra)

# Check if bio is a SpatRaster, if not, convert it
if (!inherits(bio, "SpatRaster")) {
  bio <- terra::rast(bio)
}

# Crop the bioclimatic variables to the geographic extent
bioc <- terra::crop(x = bio, y = geographic.extent)

# Extract the environmental data for occurrence points
ex <- terra::extract(bioc, occurdata2_terra)

# Convert the extracted data to a data frame
ex_df <- as.data.frame(ex)

# Check for collinearity using VIF
vif_results <- usdm::vif(ex_df)

# Remove collinearity with vifstep
v1 <- usdm::vifstep(ex_df)

# Exclude collinear variables from the bioclimatic data
# Convert bioc to RasterStack for compatibility with usdm functions
bioc_stack <- raster::stack(bioc)

# Exclude variables identified as collinear by vifstep
biom <- usdm::exclude(bioc_stack, v1)
```
...
#create model
m1 <- sdm(species~., d1, methods=c('fda', 'glm', 'gam', 'mars', 'rf', 'brt'), replication='cv', cv.folds=5, n=1)
...
pr <- predict(m1, newdata = biom)
plot(pr)
en1 <- sdm::ensemble(m1, newdata = pr, filename = paste(species_name,"_T1.tif", sep=""), setting=list(method='weighted', stat='tss', opt=2), overwrite = TRUE)
...
th <- getEvaluation(m1, stat = 'threshold', opt = 1)

# Convert ensemble probabilities to presence/absence using the threshold values
en_pa <- pa(en1, m1, id = "ensemble", opt = 2)
plot(en_pa)

# Alternatively, if you have specific threshold values:
en_pa_custom <- pa(en, y = th[, 2])
plot(en_pa_custom)

terra::writeRaster(en_pa, "Pediocactus.tif", overwrite=TRUE)

```

##  Plotting and predictions

Let's take a quick look at the predictions we have created. For spatial reference, I will constrain projections to UT. We can also map our function onto future environments to understand how species might respond to climate change.
...
library(tidyverse)
library(sf)
#Note: You will need to download state boundaries
#https://inbo.github.io/tutorials/tutorials/spatial_crs_coding/#ref-pebesma_r_2020
#tigris tutorial: https://www.google.com/search?q=i+can%27t+do+spatial+analysis+in+r+anymore+and+it+is+making+me+crazy&rlz=1C5CHFA_enUS998US998&oq=i+can%27t+do+spatial+analysis+in+r+anymore+and+it+is+making+me+crazy&aqs=chrome..69i57.12108j0j7&sourceid=chrome&ie=UTF-8#fpstate=ive&vld=cid:8fe93f3d,vid:GqC1HjAKui4

#this appears to be the main explanatory info
#https://r-spatial.github.io/sf/articles/sf1.html#reading-and-writing
...
USstates <- states(cb = TRUE)
az <- USstates[USstates$STUSPS == "AZ",]
plot(st_geometry(az))
azNEW <- st_transform(az, 4326)
plot(st_geometry(azNEW))
azNEW2 <- st_geometry(azNEW)
ggplot() + geom_sf(data=azNEW2) 


#maybe transfer az back to form that can be plotted
azNEWtrans <- 
pa1
en1
...
Now, let's plot those predictions against our original!
