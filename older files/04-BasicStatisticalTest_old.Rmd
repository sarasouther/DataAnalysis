...
Before we dive into individual tests, it’s important to understand the statistical framework we’ll be using: frequentist statistics.

Frequentist statistics is the most common framework in introductory courses and most published ecological and social science research. It’s based on the idea that probability reflects the long-run frequency of events if an experiment were repeated infinitely.

## Core Ideas of Frequentist Statistics

	•	We assume there is a fixed, true value for parameters (like a population mean or effect size), even if we can’t observe it directly.
	•	We collect a sample, estimate that value, and ask:
“How likely is it to observe data like this, if our null hypothesis were true?”
	•	We use p-values to determine whether our observed results are unlikely under the null hypothesis.

## Example

Let’s say we’re testing whether a new treatment increases plant growth. Under the null hypothesis, the treatment has no effect, and any differences in plant height are just due to chance.

We collect data, calculate a test statistic (like a t-value or F-value), and use that to compute a p-value—the probability of observing our data (or something more extreme) if the treatment truly had no effect:

	•	If the p-value is small (typically < 0.05), we reject the null hypothesis.
	•	If the p-value is large, we don’t have enough evidence to reject it.

Frequentist Statistics vs. Other Approaches (Optional Teaser):

	•	Frequentist = estimates are based on fixed parameters and sampling distributions
	•	Bayesian = combines prior knowledge with observed data
	•	Likelihood-based (e.g., AIC approaches) = compares models based on their fit to the data

We’ll focus on frequentist statistics in this course, particularly through the lens of Generalized Linear Models (GLMs)—but understanding the logic behind p-values, null hypotheses, and confidence intervals will serve you across all frameworks.
