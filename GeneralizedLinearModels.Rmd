...
SurvFig_Population <- ggplot(PECTISdata, aes(x=size, y=surv, group=DeerBrowseYear1)) + 
  geom_point(aes(colour=DeerBrowseYear1)) + 
  stat_smooth(method="glm", formula=y~x, method.args = "binomial", se=FALSE, aes(colour=DeerBrowseYear1), size=0.5) +
  ylab("Survival (Proportion)") + 
  xlab("Height 2021 (cm)") + 
  scale_color_economist("Browse intensity") +
  #scale_colour_discrete("Browsed (N/Y)") +
  #annotate("text", x = 0, y = 1.05, label = paste("A)"), hjust=0, size = 5) +
  #scale_x_continuous(breaks=seq(0,100,5)) +
  facet_wrap(~Population) +
  pubthemeleg; SurvFig_Population
