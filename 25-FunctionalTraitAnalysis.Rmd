...
```{r null-model}
# Null model: shuffle traits among species
n_null <- 999
null_fdis <- matrix(NA, n_null, n_sites)

for (i in 1:n_null) {
  # Randomize trait matrix (shuffle species labels)
  Q_null <- Q[sample(nrow(Q)), ]
  rownames(Q_null) <- rownames(Q)
  
  # Calculate FDis
  fd_null <- dbFD(Q_null, L, messages = FALSE)
  null_fdis[i, ] <- fd_null$FDis
}

# Compare observed to null
obs_fdis <- fd_results$FDis

# Calculate standardized effect size (SES)
ses_fdis <- (obs_fdis - colMeans(null_fdis)) / apply(null_fdis, 2, sd)

# Results
ses_results <- data.frame(
  Site = rownames(L),
  Observed = obs_fdis,
  Null_mean = colMeans(null_fdis),
  SES = ses_fdis
)
