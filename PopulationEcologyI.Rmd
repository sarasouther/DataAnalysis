...
```{r}
#select best model - model with lowest values
so <- makeSurvObj(dataf = pectis, Formula = surv~size+size2)
```
...
fo1<-makeFecObj(pectis, Formula=fec1~1, Family = "binomial") # Intercept only model 
fo2<-makeFecObj(pectis, Formula=fec1~size, Family = "binomial") 
fo3<-makeFecObj(pectis, Formula=fec1~size+size2, Family = "binomial")

fs <- order(pectis$size)
fs.fec <- (pectis$fec1)[fs]
fs.size <- (pectis$size)[fs]
pfz <- tapply(fs.size, as.numeric(cut(fs.size, 21)), mean, na.rm = TRUE)
ps <- tapply(fs.fec, as.numeric(cut(fs.size, 21)), mean, na.rm = TRUE)
plot(as.numeric(pfz), as.numeric(ps), pch = 19, cex=1, col="blue", ylim=c(0,1),
...
fo5 <- makeFecObj(pectis, Formula=fec2~1, Family = "poisson")
fo6 <- makeFecObj(pectis, Formula=fec2~size, Family = "poisson")
fo7 <- makeFecObj(pectis, Formula=fec2~size+size2, Family = "poisson")

fs <- order(pectis$size); fs.fec2 <- (pectis$fec2)[fs]; fs.size <- (pectis$size)[fs]
pfz <- tapply(fs.size, as.numeric(cut(fs.size, 21)), mean, na.rm = TRUE) 
ps <- tapply(fs.fec2, as.numeric(cut(fs.size, 21)), mean, na.rm = TRUE) 
plot(as.numeric(pfz), as.numeric(ps), pch = 19, cex = 1, col = "blue", xlab = "size", ylab = "Number of seeds/individual") 
    y0 <- predict(fo5@fitFec[[1]], newdata = x0, type="response")
    lines(x, y0, col="red")
    y0 <- predict(fo6@fitFec[[1]], newdata = x0, type="response")
    lines(x, y0, col="green")
    y0 <- predict(fo7@fitFec[[1]], newdata = x0, type="response")
    lines(x, y0, col="blue")

legend("topleft", legend = sprintf("%s: %s = %.1f", c("1","size","size+size2"), 
        c("AIC"), c(AIC(fo5@fitFec[[1]]), AIC(fo6@fitFec[[1]]), AIC(fo7@fitFec[[1]]))), 
        col = c(2:4), lty = 1, xjust = 1, bg = "white")
```
...
```{r}
seedtotal <- sum(pectis$fec2, na.rm=T)

#for subsampling - just replace the count using the rows with the estimated number of germinants
truegerminanttotal <- NROW(pectis[is.na(pectis$size)==TRUE & is.na(pectis$sizeNext)==FALSE,"sizeNext"]) #or use count function in plyr, but haven't loaded that
omissionrate <- 1.0 #1.1 assumes 10% not found, adds 10 percent to germinant counts; next year derive some probably for missing new seedlings
germinanttotal <- round((truegerminanttotal*omissionrate), digits=0) 
a10 <- germinanttotal / seedtotal

fo <- makeFecObj(pectis, Formula=c(fec1~size, fec2~size+size2), 
                 Family = c("binomial", "poisson"), 
                 Transform=c("none", -1), # because seed production is contingent on producing flowers the second transformation is -1 (I think that I did this incorrectly in the ginseng model)
                 fecConstants = data.frame(fec3=a10),
                 meanOffspringSize=mean(pectis[is.na(pectis$size)==TRUE & is.na(pectis$sizeNext)==FALSE,"sizeNext"]), 
                 sdOffspringSize=sd(pectis[is.na(pectis$size)==TRUE & is.na(pectis$sizeNext)==FALSE,"sizeNext"]))
```
...
```{r}
#BUILD P MATRIX====================================
Pmatrix <- makeIPMPmatrix(survObj=so, growObj=go, minSize=minSize, maxSize=maxSize, nBigMatrix=nBigMatrix, correction = "constant")
#create F matrix
Fmatrix <- makeIPMFmatrix(fecObj=fo, minSize=minSize, maxSize=maxSize, nBigMatrix=nBigMatrix, correction = "constant")

IPM <- Pmatrix + Fmatrix
lambdatotpop <- Re(eigen(IPM)$values[1])
```
...
subsetlam <- data.frame(
  pectis$size,
  pectis$id,
  pectis$DeerBrowseYear1,
  pectis$BrowseLevelYear1,
  pectis$CoverYear1,
  pectis$ProtectedFRBrowseYear1,
  lamvals=NA
)

for (i in 1:nrow(pectis)) {
  subset <- pectis[-i,]
  go <- makeGrowthObj(dataf = subset, Formula = sizeNext~size+size2, regType = "constantVar", Family="gaussian")
  so <- makeSurvObj(dataf = subset, Formula = surv~size+size2)
  fo <- makeFecObj(subset, Formula=c(fec1~size, fec2~size+size2), 
                   Family = c("binomial", "poisson"), 
                   Transform=c("none", -1), # because seed production is contingent on producing flowers the second transformation is -1 (I think that I did this incorrectly in the ginseng model)
                   fecConstants = data.frame(fec3=a10),
                   meanOffspringSize=mean(subset[is.na(subset$size)==TRUE & is.na(subset$sizeNext)==FALSE,"sizeNext"]), 
                   sdOffspringSize=sd(subset[is.na(subset$size)==TRUE & is.na(subset$sizeNext)==FALSE,"sizeNext"]))
  Pmatrix <- makeIPMPmatrix(survObj=so, growObj=go, minSize=minSize, maxSize=maxSize, nBigMatrix=nBigMatrix, correction = "constant")
  Fmatrix <- makeIPMFmatrix(fecObj=fo, minSize=minSize, maxSize=maxSize, nBigMatrix=nBigMatrix, correction = "constant")
  IPM <- Pmatrix + Fmatrix
  lam <- Re(eigen(IPM)$values[1])
  print(paste(i," out of ", nrow(pectis), sep =""))
  subsetlam$lamvals[i] <- lam
}

psuedolam <- data.frame(id = subsetlam$pectis.id, size = pectis$size, DeerBrowseYear1 = subsetlam$pectis.DeerBrowseYear1, BrowseIntensityYear1 = subsetlam$pectis.BrowseLevelYear1, Cover2021 = subsetlam$pectis.CoverYear1, ProtectedFRBrowseYear1 = subsetlam$pectis.ProtectedFRBrowseYear1, psuedolamvals = lambdatotpop-subsetlam$lamvals)

theta2 <- mean(subsetlam$lamvals)
varpseudo2 <- var(subsetlam$lamvals)
CIUpper <- theta2 + qt(0.975,length(subsetlam$lamvals)-1)*sqrt(varpseudo2/length(subsetlam$lamvals)); CIUpper
CILower <- theta2 - qt(0.975,length(subsetlam$lamvals)-1)*sqrt(varpseudo2/length(subsetlam$lamvals)); CILower
lambdatotpop #lambda of entire population

populationgrowthratewCI <- cbind(lambdatotpop, CILower, CIUpper)
```
